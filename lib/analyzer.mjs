...
const JOB_DESCRIPTION_HEADINGS = new Set([
  "responsibilities",
  "qualifications",
  "requirements",
...
function splitJobDescriptionIntoRequirements(jobDescription) {
  const lines = splitLines(jobDescription)
    ...
    .filter((line) => !JOB_DESCRIPTION_HEADINGS.has(normalizeText(line)));
...
function evaluateRequirement(requirement, sections, normalizedResume) {
  ...
  if (
    matchedSignals.length >= Math.max(2, Math.ceil(signals.length / 2)) ||
    (matchedSignals.length >= 2 && sectionMatches.length >= 2)
  ) {
    label = "demonstrated";
    score = 1;
...
export function analyzeResume({
  ...
  if (trimmedJob.length < 80 || requirements.length < 2) {
    return {
      refusal: {
        title: "Analysis could not run",
        message:
